                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <select id="jenis_modal_id" name="jenis_modal_id"
                            class="flex h-10 w-full rounded-md border {{ $errors->has('jenis_modal_id') ? 'border-red-400 focus:ring-red-500' : 'border-zinc-200 focus:ring-zinc-950' }} bg-white pl-3 pr-8 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 transition-colors cursor-pointer appearance-none"
                            required>
                            <option value="" disabled {{ old('jenis_modal_id') ? '' : 'selected' }}>-- Pilih Jenis Modal --</option>
                            @foreach ($jenisModals as $kat)
                                <option value="{{ $kat->id }}" {{ old('jenis_modal_id') == $kat->id ? 'selected' : '' }}>
