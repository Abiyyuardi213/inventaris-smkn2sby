...
@section('title', 'Daftar Kategori - Inventaris SMKN 2 SBY')
...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-zinc-500">
        ...
        <span class="font-medium text-zinc-900">Kategori</span>
    </nav>

    {{-- Header Section --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Daftar Kategori</h2>
            <p class="text-sm text-zinc-500">Kelola kategori pengelompokan inventaris aset SMKN 2 Surabaya.</p>
        </div>
        <div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="text-xs uppercase bg-zinc-50 text-zinc-500 border-b border-zinc-200">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 font-semibold">Kode Kategori</th>
                ...
                <tbody class="divide-y divide-zinc-100">
                    @forelse ($kategoris as $kategori)
                        <tr class="hover:bg-zinc-50/60 transition-colors {{ $loop->even ? 'bg-zinc-50/30' : 'bg-white' }}">
                            ...
                            <td class="px-6 py-4">
                                <button onclick="copyToClipboard('{{ $kategori->id }}', this)"
                            ...
                            <td class="px-6 py-4">
                                <div class="font-medium text-zinc-900">{{ $kategori->nama_kategori }}</div>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex items-center justify-end gap-2">
                                    ...
                                    <form id="delete-form-{{ $kategori->id }}"
                                        action="{{ route('kategoris.destroy', $kategori->id) }}"
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center text-zinc-500">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-zinc-700">Belum ada data kategori</p>
                                        <p class="text-xs text-zinc-400 mt-0.5">Klik tombol "Tambah Kategori" untuk menambahkan data pertama.</p>
                                    </div>
                                    <a href="{{ route('kategoris.create') }}"
        ...
        @if ($kategoris->isNotEmpty())
            <div class="px-6 py-3 border-t border-zinc-100 bg-zinc-50/50">
                <p class="text-xs text-zinc-400">
                    Menampilkan <span class="font-medium text-zinc-600">{{ $kategoris->count() }}</span> kategori
                </p>
            </div>
        @endif
    </div>
...
<script>
    function confirmDelete(id, nama) {
        Swal.fire({
            ...
            html: `Anda akan menghapus kategori <strong>"${nama}"</strong>.<br>
                   <span class="text-sm text-gray-500">Kategori yang masih digunakan oleh inventaris tidak dapat dihapus.</span>`,
            icon: 'warning',
    ...
    function copyToClipboard(text, button) {
        ...
        const showSuccessState = () => {
            ...
            showToast('success', 'ID kategori berhasil disalin!');
        ...
        const showErrorState = (error) => {
            ...
            showToast('error', 'ID kategori gagal disalin.');
        };
