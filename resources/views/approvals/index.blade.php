...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-zinc-500">
        <a href="{{ route('dashboard') }}" class="hover:text-zinc-900 transition-colors font-sans font-medium">Dashboard</a>
        <svg class="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
        </svg>
        <span class="font-medium text-zinc-900 font-sans">Approval Pengadaan</span>
    </nav>

    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900 font-sans">Approval Usulan Pengadaan</h2>
            <p class="text-sm text-zinc-500 mt-0.5">
                @if ($pendingPengadaans->isEmpty())
                    Semua usulan telah diproses.
                @else
                    <span class="inline-flex items-center gap-1 font-semibold text-amber-600 font-sans">
        ...
        <a href="{{ route('pengadaans.index') }}"
            class="inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-950 transition-colors font-sans">
    ...
    @if ($pendingPengadaans->isEmpty())
        <div class="rounded-lg border border-zinc-200 bg-white shadow-sm px-6 py-20 text-center">
            <div class="flex flex-col items-center justify-center gap-4">
                ...
                <div>
                    <p class="text-base font-semibold text-zinc-800 font-sans">Semua usulan sudah diproses</p>
                ...
                <a href="{{ route('pengadaans.index') }}"
                    class="mt-1 inline-flex items-center gap-1.5 rounded-md border border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-50 px-4 py-2 text-sm font-medium transition-colors shadow-sm font-sans">
                    Lihat Semua Usulan
                </a>
            </div>
        </div>
            ...
            @foreach ($pendingPengadaans as $p)
                <div class="rounded-xl border border-zinc-200 bg-white shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col overflow-hidden">
                    ...
                    <div class="px-5 py-4 flex-1 space-y-3">
                        ...
                        {{-- Pengusul & Tanggal --}}
                        <div class="flex items-center gap-2 pt-1 border-t border-zinc-100 font-sans">
