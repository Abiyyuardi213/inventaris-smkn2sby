...
@section('title', 'Daftar Jenis Modal - Inventaris SMKN 2 SBY')
@section('page_title', 'Manajemen Jenis Modal')
...
<div class="space-y-6">
    ...
    <nav class="flex items-center gap-1.5 text-sm text-zinc-500">
        ...
        <span class="font-medium text-zinc-900">Jenis Modal</span>
    </nav>

    {{-- Header Section --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold tracking-tight text-zinc-900">Daftar Jenis Modal</h2>
            <p class="text-sm text-zinc-500">Kelola jenis modal pengelompokan inventaris aset SMKN 2 Surabaya.</p>
        </div>
        <div>
            <a href="{{ route('jenis-modals.create') }}"
    ...
    <div class="rounded-lg border border-zinc-200 bg-white shadow-sm overflow-hidden">
            ...
            <table class="w-full text-sm text-left">
                <thead class="text-xs uppercase bg-zinc-50 text-zinc-500 border-b border-zinc-200">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 font-semibold">Kode Jenis Modal</th>
                        <th scope="col" class="px-6 py-4 font-semibold">Nama Jenis Modal</th>
                        <th scope="col" class="px-6 py-4 font-semibold text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-100">
                    @forelse ($jenisModals as $jenisModal)
                        <tr class="hover:bg-zinc-50/60 transition-colors {{ $loop->even ? 'bg-zinc-50/30' : 'bg-white' }}">
                            ...
                            <td class="px-6 py-4">
                                <button onclick="copyToClipboard('{{ $jenisModal->id }}', this)"
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center rounded-md bg-teal-50 px-2.5 py-1 text-xs font-mono font-semibold text-teal-700 border border-teal-200/60 tracking-wide">
                                    {{ $jenisModal->kode_jenis_modal }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-medium text-zinc-900">{{ $jenisModal->nama_jenis_modal }}</div>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="inline-flex items-center justify-end gap-2">
                                    {{-- Lihat --}}
                                    <a href="{{ route('jenis-modals.show', $jenisModal->id) }}"
                                    ...
                                    {{-- Hapus --}}
                                    <form id="delete-form-{{ $jenisModal->id }}"
                                        action="{{ route('jenis-modals.destroy', $jenisModal->id) }}"
                                        method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            onclick="confirmDelete('{{ $jenisModal->id }}', '{{ addslashes($jenisModal->nama_jenis_modal) }}')"
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center text-zinc-500">
                                <div class="flex flex-col items-center justify-center gap-3">
                                    ...
                                    <div>
                                        <p class="font-semibold text-zinc-700">Belum ada data Jenis Modal</p>
                                        <p class="text-xs text-zinc-400 mt-0.5">Klik tombol "Tambah Jenis Modal" untuk menambahkan data pertama.</p>
                                    </div>
                                    <a href="{{ route('jenis-modals.create') }}"
        ...
        {{-- Table Footer --}}
        @if ($jenisModals->isNotEmpty())
            <div class="px-6 py-3 border-t border-zinc-100 bg-zinc-50/50">
                <p class="text-xs text-zinc-400">
                    Menampilkan <span class="font-medium text-zinc-600">{{ $jenisModals->count() }}</span> Jenis Modal
                </p>
            </div>
        @endif
    </div>
...
<script>
    function confirmDelete(id, nama) {
        Swal.fire({
            title: 'Hapus Jenis Modal?',
            html: `Anda akan menghapus data Jenis Modal <strong>"${nama}"</strong>.<br>
                   <span class="text-sm text-gray-500">Barang inventaris dengan Jenis Modal ini tidak dapat dihapus jika masih terkait.</span>`,
            icon: 'warning',
    ...
    function copyToClipboard(text, element) {
        navigator.clipboard.writeText(text).then(() => {
            ...
            element.classList.add('bg-zinc-950', 'text-white', 'border-zinc-950');
            
            setTimeout(() => {
                span.textContent = originalText;
                element.classList.remove('bg-zinc-950', 'text-white', 'border-zinc-950');
            }, 1500);
        });
    }
</script>
